...
def conectar():
    # Puxa os dados direto dos Secrets do Streamlit que você salvou agora!
...
<style>
    ...
    .stSelectbox div[data-baseweb="select"] { background-color: #1F1F1F; }
</style>
""", unsafe_allow_html=True)

try:
    # Conecta usando a chave dos Secrets
    gc = conectar()
    # Abre a sua planilha pelo ID
    ...
    if not df.empty:
        # Filtros de Treino e Exercício
        lista_treinos = df['Treino'].unique()
        sel_t = st.selectbox("Selecione o Treino:", lista_treinos)
        ...
        st.warning("Conectado, mas a planilha parece estar sem dados.")
